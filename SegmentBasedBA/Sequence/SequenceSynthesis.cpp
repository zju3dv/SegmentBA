...
void Sequence::AddNoiseToCamerasOrPoints(const float percent, const bool &toCams, const bool &toPts)
{
	...
	if(toCams)
	{
		...
		for(FrameIndex iFrm = 1; iFrm < nFrms; ++iFrm)
		{
			Camera &C = m_Cs[iFrm];
			C.GetCenter(center);
			center.X() = center.X() + noiseRange * 2 * (Random::GenerateProbability() - 0.5f);
			center.Y() = center.Y() + noiseRange * 2 * (Random::GenerateProbability() - 0.5f);
			center.Z() = center.Z() + noiseRange * 2 * (Random::GenerateProbability() - 0.5f);
...
void Sequence::AddNoiseToMeasurements(const float sigma, const float errTh)
{
	...
	const Point2D xMin(imgBorder, imgBorder), xMax(GetImageWidth() - imgBorder - 1, GetImageHeight() - imgBorder - 1);
...
void Sequence::AddNoiseToFrameSegments(const float percent)
{
	...
	while(iFrm2 < nFrms)
	{
		...
		iFrm1 = iFrm2;
		for(iFrm2 = iFrm1 + 1; iFrm2 < nFrms && !(m_frmStates[iFrm2] & FLAG_FRAME_STATE_SPLIT_POINT); ++iFrm2);
		if(iFrm2 != nFrms && iFrm2 != iFrm1 + 1)
			++iFrm2;
		for(iFrm = iFrm1; iFrm < iFrm2; ++iFrm)
			TransformCamera(iFrm, dT, work3m);

		const MeasurementIndex iMea1 = m_mapFrmToMea[iFrm1], iMea2 = m_mapFrmToMea[iFrm2];
...
void Sequence::SynthesizeLoop(const ushort &width, const ushort &height, const float &fx, const float &fy, const float &cx, const float &cy, const FrameIndex nFrms, 
							  const TrackIndex nTrks, const FeatureIndex nFtrsPerFrm, const float dCenterPercentMax, const float dAngleMax)
{
	...
	const MeasurementIndex nMeas = MeasurementIndex(nFrms) * MeasurementIndex(nFtrsPerFrm);
#if _DEBUG
...
//#endif
	...
	for(FrameIndex iFrm = 0; iFrm < nFrms; ++iFrm)
	{
		...
		const FrameIndex nFrmsStart = (iFrm > nFrms - trkLen) ? (trkLen - (nFrms - iFrm)) : 0;
...
void Sequence::SynthesizeLoopPlanarScene(const ushort &width, const ushort &height, const float &fx, const float &fy, const float &cx, const float &cy, const FrameIndex nFrms, 
										 const TrackIndex nTrks, const FeatureIndex nFtrsPerFrm, AlignedVector<LA::AlignedVector4f> &Pcs, const float dCenterPercentMax, 
...
void Sequence::SynthesizeTransformation(const float percentRigid, const float maxScale, SimilarityTransformation3D &S) const
{
	Point3D sum;
	sum.SetZero();
	...
	Point3D center;
	float w[3], work[24];
	w[0] = Random::GenerateProbability() > 0.5f ? percentRigid * (Random::GenerateProbability() - 0.5f) : percentRigid * (Random::GenerateProbability() - 0.5f);
	w[1] = Random::GenerateProbability() > 0.5f ? percentRigid * (Random::GenerateProbability() - 0.5f) : percentRigid * (Random::GenerateProbability() - 0.5f);
	w[2] = Random::GenerateProbability() > 0.5f ? percentRigid * (Random::GenerateProbability() - 0.5f) : percentRigid * (Random::GenerateProbability() - 0.5f);
	//memset(w, 0, sizeof(w));
...
static inline void ComputeBoundingBox(const AlignedVector<Point2D> &xs, BoundingBox2D &B)
{
	B.Initialize();
...
void Sequence::SynthesizeIntrinsicRectification(const float maxFocal, const float maxDistortion)
{
...
//#endif
	...
	else
	{
		...
		const float d = Random::GenerateFloat(-maxDistortion, maxDistortion) * fs[0] * fs[0];
		for(iFrm = 0; iFrm < nFrms; ++iFrm)
		{
...
//#endif
	...
	const float sx = B2.GetWidth() / B1.GetWidth(), sy = B2.GetHeight() / B1.GetHeight(), s = std::max(sx, sy);
	const ushort width = ushort(m_tag.GetImageWidth() * s + 0.5f), height = ushort(m_tag.GetImageHeight() * s + 0.5f);
	const float cx = m_K.cx() * (width - 1) / (m_tag.GetImageWidth() - 1), cy = m_K.cy() * (height - 1) / (m_tag.GetImageHeight() - 1);
	m_tag.SetImageSize(width, height);
...
void Sequence::SynthesizeProjectiveTransformation(const float maxVal, AbsoluteQuadric &Q)
{
	SetReferenceFrame(0);

	switch(m_intrinsicType)
	{
	case INTRINSIC_USER_FIXED:	Q.f2() = 1.0f;							break;
	case INTRINSIC_CONSTANT:	Q.f2() = m_Kr.f() * m_Kr.f();			break;
	case INTRINSIC_VARIABLE:	Q.f2() = m_Krs[0].f() * m_Krs[0].f();	break;
	}
...
void Sequence::AnalyzeError(const std::vector<float> &errs, float &avg, float &std, float &inf, uint &iMax, const float &errMedScaleFactor)
{
	...
	else
	{
		...
		const float errMax = errMedScaleFactor == FLT_MAX ? FLT_MAX : errMed * errMedScaleFactor;
...
void Sequence::AnalyzeAndPrintError(const std::vector<float> &errs, const float &errMedScaleFactor)
{
	float avg, std, inf;
	uint iMax;
...
void Sequence::PushBackCameraError(const Camera &C1, const Camera &C2, std::vector<float> &qAbsErrs, std::vector<float> &qRelErrs, std::vector<float> &wAbsErrs, 
								   std::vector<float> &wRelErrs, std::vector<float> &tAbsErrs, std::vector<float> &tRelErrs, std::vector<float> &cAbsErrs, 
...
void Sequence::PrintCameraErrors(const std::vector<float> &qAbsErrs, const std::vector<float> &qRelErrs, const std::vector<float> &wAbsErrs, 
								 const std::vector<float> &wRelErrs, const std::vector<float> &tAbsErrs, const std::vector<float> &tRelErrs, 
								 const std::vector<float> &cAbsErrs, const std::vector<float> &cRelErrs, const float errMedScaleFactor)
{
	printf("Camera absolute error\n");
	printf("  q = ");		AnalyzeAndPrintError(qAbsErrs, errMedScaleFactor);
	printf("  w = ");		AnalyzeAndPrintError(wAbsErrs, errMedScaleFactor);
	printf("  t = ");		AnalyzeAndPrintError(tAbsErrs, errMedScaleFactor);
	printf("  c = ");		AnalyzeAndPrintError(cAbsErrs, errMedScaleFactor);
	printf("Camera relative error\n");
	printf("  q = ");		AnalyzeAndPrintError(qRelErrs, errMedScaleFactor);
	printf("  w = ");		AnalyzeAndPrintError(wRelErrs, errMedScaleFactor);
	printf("  t = ");		AnalyzeAndPrintError(tRelErrs, errMedScaleFactor);
	printf("  c = ");		AnalyzeAndPrintError(cRelErrs, errMedScaleFactor);
}

void Sequence::PushBackPointError(const Point3D &X1, const Point3D &X2, std::vector<float> &absErrs, std::vector<float> &relErrs)
{
	float absErr, relErr;
...
void Sequence::PrintPointErrors(const std::vector<float> &absErrs, const std::vector<float> &relErrs, const float errMedScaleFactor)
{
	printf("Point absolute error = ");		AnalyzeAndPrintError(absErrs, errMedScaleFactor);
	printf("Point relative error = ");		AnalyzeAndPrintError(relErrs, errMedScaleFactor);
}

void Sequence::PushBackIntrinsicRectificationError(const Camera::IntrinsicParameter &Kr1, const Camera::IntrinsicParameter &Kr2, std::vector<float> &absErrs, 
												   std::vector<float> &relErrs)
{
	float absErr, relErr;
	const LA::Vector2f e(Kr1.f() - Kr2.f(), Kr1.d() - Kr2.d());
	absErr = sqrt(e.SquaredLength());
	const float len1 = sqrt(Kr1.f() * Kr1.f() + Kr1.d() * Kr1.d()), len2 = sqrt(Kr2.f() * Kr2.f() + Kr2.d() * Kr2.d()), lenMax = std::max(len1, len2);
	if(lenMax == 0)
		relErr = 0;
	else
...
void Sequence::PushBackIntrinsicRectificationError(const float &f1, const float &f2, std::vector<float> &absErrs, std::vector<float> &relErrs)
{
	float absErr, relErr;
	absErr = fabs(f1 - f2);
...
void Sequence::PrintIntrinsicRectificationErrors(const std::vector<float> &absErrs, const std::vector<float> &relErrs, const float errMedScaleFactor /* = 2.0f */)
{
	printf("Intrinsic absolute error = ");		AnalyzeAndPrintError(absErrs, errMedScaleFactor);
	printf("Intrinsic relative error = ");		AnalyzeAndPrintError(relErrs, errMedScaleFactor);
}

void Sequence::PushBackProjectiveMatrixError(const ProjectiveMatrix &P1, const ProjectiveMatrix &P2, std::vector<float> &absErrs, std::vector<float> &relErrs)
{
	LA::AlignedVector11f p1, p2, e;
	const  ENFT_SSE::__m128 s1 =  ENFT_SSE::_mm_set1_ps(1 / P1.FrobeniusNorm()), s2 =  ENFT_SSE::_mm_set1_ps(1 / P2.FrobeniusNorm());
	p1.v0123() =  ENFT_SSE::_mm_mul_ps(s1, P1.M_00_01_02_03());		p2.v0123() =  ENFT_SSE::_mm_mul_ps(s2, P2.M_00_01_02_03());
	p1.v4567() =  ENFT_SSE::_mm_mul_ps(s1, P1.M_10_11_12_13());		p2.v4567() =  ENFT_SSE::_mm_mul_ps(s2, P2.M_10_11_12_13());
	p1.v8910x() =  ENFT_SSE::_mm_mul_ps(s1, P1.M_20_21_22_23());		p2.v8910x() =  ENFT_SSE::_mm_mul_ps(s2, P2.M_20_21_22_23());
	p1.reserve() = 0.0f;									p2.reserve() = 0.0f;
	LA::AmB(p1, p2, e);

	float absErr, relErr;
	absErr = fabs(e.SquaredLength());
	const float len1 = sqrt(p1.SquaredLength()), len2 = sqrt(p2.SquaredLength()), lenMax = std::max(len1, len2);
	if(lenMax == 0)
		relErr = 0;
	else
		relErr = absErr / lenMax;
...
void Sequence::PrintProjectiveMatrixErrors(const std::vector<float> &absErrs, const std::vector<float> &relErrs, const float errMedScaleFactor)
{
	printf("Projective absolute error = ");		AnalyzeAndPrintError(absErrs, errMedScaleFactor);
	printf("Projective relative error = ");		AnalyzeAndPrintError(relErrs, errMedScaleFactor);
}