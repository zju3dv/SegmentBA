...
#define FLAG_MEASUREMENT_STATE_KLT		4
...
public:
	...
	inline void Get(FrameIndex &iFrm, FeatureIndex &iFtr) const { iFrm = m_iFrm; iFtr = m_iFtr; }
	inline void SetFrameIndex	(const FrameIndex	&iFrm) { m_iFrm = iFrm; }
	inline void SetFeatureIndex	(const FeatureIndex	&iFtr) { m_iFtr = iFtr; }
	inline void Set(const FrameIndex &iFrm, const FeatureIndex &iFtr) { m_iFrm = iFrm; m_iFtr = iFtr; }
	inline void Invalidate() { m_iFrm = INVALID_FRAME_INDEX; }
	inline bool IsValid()	const { return m_iFrm != INVALID_FRAME_INDEX; }
	inline bool IsInvalid() const { return m_iFrm == INVALID_FRAME_INDEX; }
private:
	FrameIndex		m_iFrm;
	FeatureIndex	m_iFtr;
};
...
public:
	...
	inline const FeatureIndex&	GetFeatureIndex	() const { return m_iFtr; }
	inline void Get(TrackIndex &iTrk, FeatureIndex &iFtr) const { iTrk = m_iTrk; iFtr = m_iFtr; }
	inline void SetTrackIndex(const TrackIndex	&iTrk) { m_iTrk = iTrk; }
	inline void SetFeatureIndex	(const FeatureIndex		&iFtr) { m_iFtr = iFtr; }
	inline void Set(const TrackIndex &iTrk, const FeatureIndex &iFtr) { m_iTrk = iTrk; m_iFtr = iFtr; }
	inline void Invalidate() { m_iTrk = INVALID_TRACK_INDEX; }
	inline bool IsValid()	const { return m_iTrk != INVALID_TRACK_INDEX; }
	inline bool IsInvalid() const { return m_iTrk == INVALID_TRACK_INDEX; }
	inline bool operator < (const TrackFeatureIndex &iTrkFtr) const { return m_iTrk < iTrkFtr.m_iTrk || m_iTrk == iTrkFtr.m_iTrk && m_iFtr < iTrkFtr.m_iFtr; }
private:
	TrackIndex		m_iTrk;
	FeatureIndex	m_iFtr;
};
...
public:
	...
	// Basic
	//////////////////////////////////////////////////////////////////////////
	Sequence() : m_K(*(IntrinsicMatrix *) _aligned_malloc(sizeof(IntrinsicMatrix), SSE_ALIGNMENT)), m_intrinsicType(INTRINSIC_USER_FIXED), m_measNormalized(false) {}
	~Sequence() { Clear(); _aligned_free(&m_K); }
	inline const SequenceTag& GetTag() const { return m_tag; }
	inline void SetDirectory(const std::string &seqDir) { m_tag.SetDirectory(seqDir); }
	inline const std::string& GetDirectory() const { return m_tag.GetDirectory(); }
	inline void SetImageSize(const ushort &width, const ushort &height) { m_tag.SetImageSize(width, height); }
	inline const ushort& GetImageWidth () const { return m_tag.GetImageWidth (); }
	inline const ushort& GetImageHeight() const { return m_tag.GetImageHeight(); }
	inline const std::string& GetImageFileName(const FrameIndex &iFrm) const { return m_tag.GetImageFileName(iFrm); }
	inline const std::string& GetSequenceName() const { return m_tag.GetSequenceName(); }
	inline const int& GetStartFrame() const { return m_tag.GetStartFrame(); }
	inline const int& GetStepFrame() const { return m_tag.GetStepFrame(); }
	inline const int& GetEndFrame() const { return m_tag.GetEndFrame(); }
	inline void SetImageFileName(const FrameIndex &iFrm, const std::string &imgFileName) { m_tag.SetImageFileName(iFrm, imgFileName); }
	inline void PushBackImageFileName(const std::string &imgFileName) { m_tag.PushBackImageFileName(imgFileName); }
	inline void SetIntrinsicMatrix(const float &fx, const float &fy, const float &cx, const float &cy) { m_K.Set(fx, fy, cx, cy); }
	inline void SetIntrinsicMatrix(const IntrinsicMatrix &K) { m_K = K; }
	inline const IntrinsicMatrix& GetIntrinsicMatrix() const { return m_K; }
	inline void SetIntrinsicRectification(const Camera::IntrinsicParameter &Kr) { m_Kr = Kr; ComputeProjectiveMatrixes(); }
	inline const Camera::IntrinsicParameter& GetIntrinsicRectification() const { return m_Kr; }
	inline const AlignedVector<Camera::IntrinsicParameter>& GetIntrinsicRectifications() const { return m_Krs; }
	inline const Camera::IntrinsicParameter& GetIntrinsicRectification(const FrameIndex &iFrm) const { return m_Krs[iFrm]; }
	inline void SetIntrinsicType(const IntrinsicType &intrinsicType) { if((m_intrinsicType = intrinsicType) == INTRINSIC_VARIABLE) m_Krs.Resize(m_Cs.Size()); }
	inline const IntrinsicType& GetIntrinsicType() const { return m_intrinsicType; }
	...
	// Frame
	//////////////////////////////////////////////////////////////////////////
	inline FrameIndex GetFramesNumber() const { return FrameIndex(m_frmStates.size()); }
	inline FrameIndex GetFramesNumberTotal() const { return m_tag.GetFramesNumber(); }
	inline FrameIndex GetCamerasNumber() const { return FrameIndex(m_Cs.Size()); }
	inline const AlignedVector<Camera>& GetCameras() const { return m_Cs; }
	inline const Camera& GetCamera(const FrameIndex &iFrm) const { return m_Cs[iFrm]; }
	inline void SwapCameras(AlignedVector<Camera> &Cs) { m_Cs.Swap(Cs); }
	inline const AlignedVector<ProjectiveMatrix>& GetProjectiveMatrixes() const { return m_Ps; }
	inline const ProjectiveMatrix& GetProjectiveMatrix(const FrameIndex &iFrm) const { return m_Ps[iFrm]; }
	inline const FrameState& GetFrameState(const FrameIndex &iFrm) const { return m_frmStates[iFrm]; }
	inline ubyte IsFrameKeyFrame(const FrameIndex &iFrm) const { return (m_frmStates[iFrm] & FLAG_FRAME_STATE_KEY_FRAME); }
	inline ubyte IsFrameSolved(const FrameIndex &iFrm) const { return (m_frmStates[iFrm] & FLAG_FRAME_STATE_SOLVED); }
	inline const MeasurementIndex& GetFrameFirstMeasurementIndex(const FrameIndex &iFrm) const { return m_mapFrmToMea[iFrm]; }
	inline FeatureIndex GetFrameFeaturesNumber(const FrameIndex &iFrm) const { return m_mapFrmToMea[iFrm + 1] - m_mapFrmToMea[iFrm]; }
	inline const Point2D* GetFrameFeatures(const FrameIndex &iFrm) const { return &m_xs[m_mapFrmToMea[iFrm]]; }
	inline const Point2D& GetFrameFeature(const FrameIndex &iFrm, const FeatureIndex &iFtr) const { return m_xs[m_mapFrmToMea[iFrm] + iFtr]; }
	inline const MeasurementState* GetFrameMeasurementStates(const FrameIndex &iFrm) const { return &m_meaStates[m_mapFrmToMea[iFrm]]; }
	inline const TrackIndex* GetFrameTrackIndexes(const FrameIndex &iFrm) const { return &m_mapMeaToTrk[m_mapFrmToMea[iFrm]]; }
	inline const TrackIndex& GetFrameFeatureTrackIndex(const FrameIndex &iFrm, const FeatureIndex &iFtr) const { return m_mapMeaToTrk[m_mapFrmToMea[iFrm] + iFtr]; }
	inline MeasurementIndex GetFrameFeatureMeasurementIndex(const FrameIndex &iFrm, const FeatureIndex &iFtr) const { return m_mapFrmToMea[iFrm] + iFtr; }
#if DESCRIPTOR_TRACK == 0
	inline const Descriptor* GetFrameDescriptors(const FrameIndex &iFrm) const { return &m_descs[m_mapFrmToMea[iFrm]]; }
	inline		 Descriptor* GetFrameDescriptors(const FrameIndex &iFrm)	   { return &m_descs[m_mapFrmToMea[iFrm]]; }
#endif
	inline void SetFramesNumber() { const FrameIndex nFrms = m_tag.GetFramesNumber(); m_frmStates.resize(nFrms, FLAG_FRAME_STATE_DEFAULT); m_mapFrmToMea.resize(nFrms + 1, 0); }
	...
	inline void SetFrameState(const FrameIndex &iFrm, const FrameState &frmState) { m_frmStates[iFrm] = frmState; }
	inline void SetFrameStates(const FrameState &frmState) { m_frmStates.assign(GetFramesNumber(), frmState); }
	inline void MarkFrameKeyFrame(const FrameIndex &iFrm) { m_frmStates[iFrm] |= FLAG_FRAME_STATE_KEY_FRAME; }
	inline void UnmarkFrameKeyFrame(const FrameIndex &iFrm) { m_frmStates[iFrm] &= ~FLAG_FRAME_STATE_KEY_FRAME; }
	inline void MarkFrameInitial(const FrameIndex &iFrm) { m_frmStates[iFrm] |= FLAG_FRAME_STATE_INITIAL; }
	inline void MarkFrameSolved(const FrameIndex &iFrm) { m_frmStates[iFrm] |= FLAG_FRAME_STATE_SOLVED; }
	inline void MarkFrameSplitPoint(const FrameIndex &iFrm) { m_frmStates[iFrm] |= FLAG_FRAME_STATE_SPLIT_POINT; }
	inline void UnmarkFrameSplitPoint(const FrameIndex &iFrm) { m_frmStates[iFrm] &= ~FLAG_FRAME_STATE_SPLIT_POINT; }
	inline void MarkFrameInterpolated(const FrameIndex &iFrm) { m_frmStates[iFrm] |= FLAG_FRAME_STATE_INTERPOLATED; }
	...
	void GetFrameFeaturesAndDescriptors(const FrameIndex &iFrm, const FeatureIndexList &iFtrs, AlignedVector<Point2D> &ftrs, AlignedVector<Descriptor> &descs) const;
	...
	void GetProjectiveMatrixEstimatorDataMarkedTrack(const FrameIndex &iFrm, ProjectiveMatrixEstimatorData &data, const std::vector<bool> &trkMarks, const bool rectData) const;
...
#endif
	void PushBackFrame(const Camera &C, const FrameState &frmState, const AlignedVector<Point2D> &xs, const TrackIndexList &iTrks, const MeasurementStateList &meaStates, 
	...
	// Track
	//////////////////////////////////////////////////////////////////////////
	inline TrackIndex GetTracksNumber() const { return TrackIndex(m_trkStates.size()); }
	inline TrackIndex GetPointsNumber() const { return TrackIndex(m_Xs.Size()); }
	inline const AlignedVector<Point3D>& GetPoints() const { return m_Xs; }
	inline const Point3D& GetPoint(const TrackIndex &iTrk) const { return m_Xs[iTrk]; }
	inline const TrackState& GetTrackState(const TrackIndex &iTrk) const { return m_trkStates[iTrk]; }
	inline const TrackStateList& GetTrackStates() const { return m_trkStates; }
#if DESCRIPTOR_TRACK
	inline TrackIndex GetDescriptorsNumber() const { return TrackIndex(m_descs.Size()); }
	inline const Descriptor& GetDescriptor(const TrackIndex &iTrk) const { return m_descs[iTrk]; }
	inline const AlignedVector<Descriptor>& GetDescriptors() const { return m_descs; }
#endif
	inline TrackIndex GetTrackColorsNumber() const { return TrackIndex(m_trkClrs.size()); }
	inline const CVD::Rgb<ubyte>& GetTrackColor(const TrackIndex &iTrk) const { return m_trkClrs[iTrk]; }
	inline const std::vector<CVD::Rgb<ubyte> >& GetTrackColors() const { return m_trkClrs; }
	inline FrameIndex GetTrackLength(const TrackIndex &iTrk) const { return FrameIndex(m_mapTrkToMea[iTrk].size()); }
	inline const MeasurementIndexList& GetTrackMeasurementIndexList(const TrackIndex &iTrk) const { return m_mapTrkToMea[iTrk]; }
	inline const FrameIndex GetTrackFirstFrameIndex(const TrackIndex &iTrk) const { return m_mapTrkToMea[iTrk].empty() ? INVALID_FRAME_INDEX : m_mapMeaToFrm[m_mapTrkToMea[iTrk].front()]; }
	inline const FrameIndex GetTrackLastFrameIndex(const TrackIndex &iTrk) const { return m_mapTrkToMea[iTrk].empty() ? INVALID_FRAME_INDEX : m_mapMeaToFrm[m_mapTrkToMea[iTrk].back()]; }
	inline const MeasurementIndex GetTrackFirstMeasurementIndex(const TrackIndex &iTrk) const { return m_mapTrkToMea[iTrk].empty() ? INVALID_MEASUREMENT_INDEX : m_mapTrkToMea[iTrk].front(); }
	inline const MeasurementIndex GetTrackLastMeasurementIndex(const TrackIndex &iTrk) const { return m_mapTrkToMea[iTrk].empty() ? INVALID_MEASUREMENT_INDEX : m_mapTrkToMea[iTrk].back(); }
	inline void MarkTrackInitial		(const TrackIndex &iTrk) { m_trkStates[iTrk] |= FLAG_TRACK_STATE_INITIAL; }
	inline void MarkTrackSolved			(const TrackIndex &iTrk) { m_trkStates[iTrk] |= FLAG_TRACK_STATE_SOLVED; }
	inline void MarkTrackInlier			(const TrackIndex &iTrk) { m_trkStates[iTrk] |= FLAG_TRACK_STATE_INLIER; }
	inline void MarkTrackSolvedAndInlier(const TrackIndex &iTrk) { m_trkStates[iTrk] |= (FLAG_TRACK_STATE_SOLVED | FLAG_TRACK_STATE_INLIER); }
	inline void MarkTrackMerged			(const TrackIndex &iTrk) { m_trkStates[iTrk] |= FLAG_TRACK_STATE_MERGED; }
	inline void MarkTrackCommon			(const TrackIndex &iTrk) { m_trkStates[iTrk] |= FLAG_TRACK_STATE_COMMON; }
	inline void MarkTrackIndividual		(const TrackIndex &iTrk) { m_trkStates[iTrk] &= ~FLAG_TRACK_STATE_COMMON; }
	inline void MarkTrackCommonInlier	(const TrackIndex &iTrk) { m_trkStates[iTrk] &= ~FLAG_TRACK_STATE_COMMON_OUTLIER; }
	inline void MarkTrackCommonOutlier	(const TrackIndex &iTrk) { m_trkStates[iTrk] |= FLAG_TRACK_STATE_COMMON_OUTLIER; }
	inline void SetTracksNumber(const TrackIndex &nTrks) { m_mapTrkToMea.resize(nTrks, MeasurementIndexList()); m_trkStates.resize(nTrks); }
	inline void SetPoint(const TrackIndex &iTrk, const Point3D &X) { m_Xs[iTrk] = X; }
	inline void SetPoints(const AlignedVector<Point3D> &Xs) { m_Xs.Resize(Xs.Size()); m_Xs.CopyFrom(Xs.Data()); }
	inline void SetTrackState(const TrackIndex &iTrk, const TrackState &trkState) { m_trkStates[iTrk] = trkState; }
	inline void SetTrackStates(const TrackStateList &trkStates) { m_trkStates = trkStates; }
#if DESCRIPTOR_TRACK
	inline void SetDescriptorsNumber(const TrackIndex &nTrks) { m_descs.Resize(nTrks); }
	inline void SetDescriptor(const TrackIndex &iTrk, const Descriptor &desc) { m_descs[iTrk] = desc; }
	inline void ClearDescriptors() { m_descs.Clear(); }
#endif
	inline void SetTrackColorsNumber(const TrackIndex &nTrks) { m_trkClrs.resize(nTrks); }
	inline void SetTrackColor(const TrackIndex &iTrk, const CVD::Rgb<ubyte> &clr) { m_trkClrs[iTrk] = clr; }
	inline void PushBackTrackColor(const CVD::Rgb<ubyte> &clr) { m_trkClrs.push_back(clr); }
	inline void SetTrackColors(const std::vector<CVD::Rgb<ubyte> > &trkClrs) { m_trkClrs = trkClrs; }
	...
	inline MeasurementIndex GetMeasurementsNumber() const { return MeasurementIndex(m_meaStates.size()); }
	inline const Point2D& GetMeasurement(const MeasurementIndex &iMea) const { return m_xs[iMea]; }
	inline const AlignedVector<Point2D>& GetMeasurements() const { return m_xs; }
	inline const MeasurementState& GetMeasurementState(const MeasurementIndex &iMea) const { return m_meaStates[iMea]; }
	inline const MeasurementStateList& GetMeasurementStates() const { return m_meaStates; }
	inline FrameIndex GetMeasurementFrameIndex(const MeasurementIndex &iMea) const { return m_mapMeaToFrm[iMea]; }
	inline TrackIndex GetMeasurementTrackIndex(const MeasurementIndex &iMea) const { return m_mapMeaToTrk[iMea]; }
	inline FeatureIndex GetMeasurementFrameFeatureIndex(const MeasurementIndex &iMea) const { return iMea - m_mapFrmToMea[m_mapMeaToFrm[iMea]]; }
	inline void MarkMeasurementInlier(const MeasurementIndex &iMea) { m_meaStates[iMea] &= ~FLAG_MEASUREMENT_STATE_OUTLIER; }
	inline void MarkMeasurementOutlier(const MeasurementIndex &iMea) { m_meaStates[iMea] |= FLAG_MEASUREMENT_STATE_OUTLIER; }
	inline const bool& AreMeasurementsNormalized() const { return m_measNormalized; }
	inline void SetMeasurement(const MeasurementIndex &iMea, const Point2D &x) { m_xs[iMea] = x; }
	inline void SetMeasurementState(const MeasurementIndex &iMea, const MeasurementState &meaState) { m_meaStates[iMea] = meaState; }
	inline void SwapMeasurements(AlignedVector<Point2D> &xs) { m_xs.Swap(xs); }
	inline void ReserveMeasurements(const MeasurementIndex &nMeas) { m_xs.Reserve(nMeas); }
#if DESCRIPTOR_TRACK == 0
	inline TrackIndex GetDescriptorsNumber() const { return TrackIndex(m_descs.Size()); }
	inline const Descriptor& GetDescriptor(const MeasurementIndex &iMea) const { return m_descs[iMea]; }
	inline const AlignedVector<Descriptor>& GetDescriptors() const { return m_descs; }
	inline void SetDescriptor(const MeasurementIndex &iMea, const Descriptor &desc) { m_descs[iMea] = desc; }
	inline void SetDescriptorsNumber(const MeasurementIndex &nMeas) { m_descs.Resize(nMeas); }
	inline void ReserveDescriptors(const MeasurementIndex &nMeas) { m_descs.Reserve(nMeas); }
	inline void ClearDescriptors() { m_descs.Clear(); }
#endif
	...
	// Data association
	//////////////////////////////////////////////////////////////////////////
	inline const FrameMeasurementMap& GetFrameMeasurementMap() const { return m_mapFrmToMea; }
	inline const TrackMeasurementMap& GetTrackMeasurementMap() const { return m_mapTrkToMea; }
	inline const MeasurementFrameMap& GetMeasurementFrameMap() const { return m_mapMeaToFrm; }
	inline const MeasurementTrackMap& GetMeasurementTrackMap() const { return m_mapMeaToTrk; }
	...
	// Transformation
	//////////////////////////////////////////////////////////////////////////
	inline void TransformCamera(const FrameIndex &iFrm, const RigidTransformation3D &T,  ENFT_SSE::__m128* const &work3) { T.Apply(m_Cs[iFrm], m_Cs[iFrm], work3); }
	inline void TransformCamera(const FrameIndex &iFrm, const SimilarityTransformation3D &S,  ENFT_SSE::__m128* const &work3) { S.Apply(m_Cs[iFrm], m_Cs[iFrm], work3); }
	inline void ScaleCamera(const FrameIndex &iFrm, const float &scale) { m_Cs[iFrm].Scale(scale); }
	inline void TransformPoint(const TrackIndex &iTrk, const RigidTransformation3D &T,  ENFT_SSE::__m128* const &work1) { T.Apply(m_Xs[iTrk], work1); }
	inline void TransformPoint(const TrackIndex &iTrk, const SimilarityTransformation3D &S,  ENFT_SSE::__m128* const &work1) { S.Apply(m_Xs[iTrk], work1); }
	inline void ScalePoint(const TrackIndex &iTrk, const  ENFT_SSE::__m128 &scale) { m_Xs[iTrk] *= scale; }
	...
	bool SaveCameraTrackAct(const char *file);
	bool LoadAct(const char *fileName, const bool normalizeMeas = true, const bool loadClr = false, const bool skipCam = false);
	bool LoadCameraTrackAct(const char* fileName, const bool normalizeMeas, const bool loadClr, const bool skipCam);
