...
public:
	Match() {}
	Match(const TYPE &idx1, const TYPE &idx2) { Set(idx1, idx2); }
	inline const TYPE& GetIndex1() const { return m_idx1; }
	inline const TYPE& GetIndex2() const { return m_idx2; }
	inline void Get(TYPE &idx1, TYPE &idx2) const { idx1 = m_idx1; idx2 = m_idx2; }
	inline void Set(const TYPE &idx1, const TYPE &idx2) { m_idx1 = idx1; m_idx2 = idx2; }
	inline void SetIndex1(const TYPE &idx1) { m_idx1 = idx1; }
	inline void SetIndex2(const TYPE &idx2) { m_idx2 = idx2; }
	inline void IncreaseIndex1(const TYPE &incr) { m_idx1 += incr; }
	inline void IncreaseIndex2(const TYPE &incr) { m_idx2 += incr; }
	inline void Invert(TYPE &tmp) { SWAP(m_idx1, m_idx2, tmp); }
protected:
	TYPE m_idx1, m_idx2;
};
...
public:
	...
	inline void Invalidate() { m_idx = -1; }
	inline bool IsInvalid() const { return m_idx < 0; }
	inline const float& GetIndex() const { return m_idx; }
	inline const ushort& GetIndex1() const { return m_idx1; }
	inline const ushort& GetIndex2() const { return m_idx2; }
	inline const float& GetScore() const { return m_score; }
	inline void Get(Match<ushort> &match) const { match.Set(m_idx1, m_idx2); }
	inline void Get(ushort &idx1, ushort &idx2) const { idx1 = m_idx1; idx2 = m_idx2; }
	inline void Set(const ushort &idx1, const ushort &idx2, const float &score) { m_idx1 = idx1; m_idx2 = idx2; m_score = score; }
	inline void SetIndex1(const ushort &idx1) { m_idx1 = idx1; }
	inline void SetIndex2(const ushort &idx2) { m_idx2 = idx2; }
	inline void IncreaseIndex1(const ushort &incr) { m_idx1 += incr; }
	inline void IncreaseIndex2(const ushort &incr) { m_idx2 += incr; }
	inline bool operator < (const ScoredMatch &match) const { return m_score > match.m_score; }
protected:
	union
	{
		float m_idx;
		struct { ushort m_idx1, m_idx2; };
	};
	float m_score;
};
...
public:
	...
	MatchSet2D() { memset(this, 0, 6); }
	inline const ushort& Size() const { return m_N; }
	...
	inline void SetMatches(const AlignedVector<Point2D> &ftrs1, const std::vector<ushort> &iFtrs1, const std::vector<FeatureKlt> &ftrs2, std::vector<ushort> &orders)
	{
		const ushort nFtrs2 = ushort(ftrs2.size());
		Resize(nFtrs2);
		m_scores.resize(nFtrs2);
		ushort iFtr1, iFtr2;
		for(iFtr2 = 0; iFtr2 < nFtrs2; ++iFtr2)
		{
			iFtr1 = iFtrs1[iFtr2];
			SetMatch(iFtr2, ftrs1[iFtr1], ftrs2[iFtr2]);
			m_scores[iFtr2] = std::make_pair(ftrs2[iFtr2].GetSAD(), iFtr2);
		}
		FinishSettingMatches();

		std::sort(m_scores.begin(), m_scores.end());
		orders.resize(nFtrs2);
		for(iFtr2 = 0; iFtr2 < nFtrs2; ++iFtr2)
			orders[iFtr2] = m_scores[iFtr2].second;
	}
	...
	inline void Rectify(const Camera::IntrinsicParameter &Kr1, const Camera::IntrinsicParameter &Kr2)
	{
		...
		const  ENFT_SSE::__m128 one =  ENFT_SSE::_mm_set1_ps(1.0f), d1 =  ENFT_SSE::_mm_set1_ps(Kr1.d()), f1inv =  ENFT_SSE::_mm_set1_ps(1 / Kr1.f()), d2 =  ENFT_SSE::_mm_set1_ps(Kr2.d()), f2inv =  ENFT_SSE::_mm_set1_ps(1 / Kr2.f());
		const ushort nPacksCeil = m_nRems == 0 ? m_nPacks : m_nPacks + 4;
		for(ushort i0 = 0, i1 = 1, i2 = 2, i3 = 3; i0 < nPacksCeil; i0 += 4, i1 += 4, i2 += 4, i3 += 4)
		{
			 ENFT_SSE::__m128 &u1 = m_data[i0], &v1 = m_data[i1], &u2 = m_data[i2], &v2 = m_data[i3];
	...
	inline void GetMatch(const ushort &i, Point2D &x1, Point2D &x2) const {	GetMatch(i, x1.x(), x1.y(), x2.x(), x2.y()); }
	...
	inline void Normalize( ENFT_SSE::__m128 *work6)
	{
		...
		m_mean_u1v1u2v2 =  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set_ps(ENFT_SSE::SSE::Sum0123(work6[3]), ENFT_SSE::SSE::Sum0123(work6[2]), ENFT_SSE::SSE::Sum0123(work6[1]), ENFT_SSE::SSE::Sum0123(work6[0])),  ENFT_SSE::_mm_set1_ps(1.0f / m_N));
		...
		for(ushort iPack = 0; iPack < m_nPacks; iPack += 4)
		{
			...
			work6[4] =  ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_sqrt_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps(m_data[iPack    ], m_data[iPack    ]),  ENFT_SSE::_mm_mul_ps(m_data[iPack + 1], m_data[iPack + 1]))), work6[4]);
			work6[5] =  ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_sqrt_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps(m_data[iPack + 2], m_data[iPack + 2]),  ENFT_SSE::_mm_mul_ps(m_data[iPack + 3], m_data[iPack + 3]))), work6[5]);
		}
		for(ushort iRem = 0; iRem < m_nRems; ++iRem)
		{
	...
	inline const  ENFT_SSE::__m128& mean_u1v1u2v2() const { return m_mean_u1v1u2v2; }
	inline const float& scale1() const { return m_scale1; }
	inline const float& scale2() const { return m_scale2; }
	inline const ushort& GetPacksNumber	   () const { return m_nPacks; }
	inline const ushort& GetRemindersNumber() const { return m_nRems; }
	const  ENFT_SSE::__m128& GetPack(const ushort &iPack) const { return m_data[iPack]; }
	const Point2D& GetReminder1(const ushort &iRem) const { return m_rems1[iRem]; }
	const Point2D& GetReminder2(const ushort &iRem) const { return m_rems2[iRem]; }
...
public:
	...
	inline const  ENFT_SSE::__m128& u1() const { return m_u1; }
	inline const  ENFT_SSE::__m128& v1() const { return m_v1; }
	inline const  ENFT_SSE::__m128& u2() const { return m_u2; }
	inline const  ENFT_SSE::__m128& v2() const { return m_v2; }inline const  ENFT_SSE::__m128& mean_u1v1u2v2() const { return m_mean_u1v1u2v2; }
	inline const float& scale1() const { return m_scale1; }
	inline const float& scale2() const { return m_scale2; }
	inline void Normalize( ENFT_SSE::__m128 *work)
	{
		m_mean_u1v1u2v2 =  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set_ps(ENFT_SSE::SSE::Sum0123(m_v2), ENFT_SSE::SSE::Sum0123(m_u2), ENFT_SSE::SSE::Sum0123(m_v1), ENFT_SSE::SSE::Sum0123(m_u1)),  ENFT_SSE::_mm_set1_ps(0.25f));
...
public:
	...
	inline const  ENFT_SSE::__m128& u1_0123() const { return m_u1_0123; }				inline  ENFT_SSE::__m128& u1_0123() { return m_u1_0123; }
	inline const  ENFT_SSE::__m128& v1_0123() const { return m_v1_0123; }				inline  ENFT_SSE::__m128& v1_0123() { return m_v1_0123; }
	inline const  ENFT_SSE::__m128& u2_0123() const { return m_u2_0123; }				inline  ENFT_SSE::__m128& u2_0123() { return m_u2_0123; }
	inline const  ENFT_SSE::__m128& v2_0123() const { return m_v2_0123; }				inline  ENFT_SSE::__m128& v2_0123() { return m_v2_0123; }
	inline const float& u1_0() const { return m_u1_0123.m128_f32[0]; }		inline float& u1_0() { return m_u1_0123.m128_f32[0]; }
	inline const float& v1_0() const { return m_v1_0123.m128_f32[0]; }		inline float& v1_0() { return m_v1_0123.m128_f32[0]; }
	inline const float& u2_0() const { return m_u2_0123.m128_f32[0]; }		inline float& u2_0() { return m_u2_0123.m128_f32[0]; }
	inline const float& v2_0() const { return m_v2_0123.m128_f32[0]; }		inline float& v2_0() { return m_v2_0123.m128_f32[0]; }
	inline const float& u1_1() const { return m_u1_0123.m128_f32[1]; }		inline float& u1_1() { return m_u1_0123.m128_f32[1]; }
	inline const float& v1_1() const { return m_v1_0123.m128_f32[1]; }		inline float& v1_1() { return m_v1_0123.m128_f32[1]; }
	inline const float& u2_1() const { return m_u2_0123.m128_f32[1]; }		inline float& u2_1() { return m_u2_0123.m128_f32[1]; }
	inline const float& v2_1() const { return m_v2_0123.m128_f32[1]; }		inline float& v2_1() { return m_v2_0123.m128_f32[1]; }
	inline const float& u1_2() const { return m_u1_0123.m128_f32[2]; }		inline float& u1_2() { return m_u1_0123.m128_f32[2]; }
	inline const float& v1_2() const { return m_v1_0123.m128_f32[2]; }		inline float& v1_2() { return m_v1_0123.m128_f32[2]; }
	inline const float& u2_2() const { return m_u2_0123.m128_f32[2]; }		inline float& u2_2() { return m_u2_0123.m128_f32[2]; }
	inline const float& v2_2() const { return m_v2_0123.m128_f32[2]; }		inline float& v2_2() { return m_v2_0123.m128_f32[2]; }
	inline const float& u1_3() const { return m_u1_0123.m128_f32[3]; }		inline float& u1_3() { return m_u1_0123.m128_f32[3]; }
	inline const float& v1_3() const { return m_v1_0123.m128_f32[3]; }		inline float& v1_3() { return m_v1_0123.m128_f32[3]; }
	inline const float& u2_3() const { return m_u2_0123.m128_f32[3]; }		inline float& u2_3() { return m_u2_0123.m128_f32[3]; }
	inline const float& v2_3() const { return m_v2_0123.m128_f32[3]; }		inline float& v2_3() { return m_v2_0123.m128_f32[3]; }
	inline const float& u1_4() const { return m_u1_4; }						inline float& u1_4() { return m_u1_4; }
	inline const float& v1_4() const { return m_v1_4; }						inline float& v1_4() { return m_v1_4; }
	inline const float& u2_4() const { return m_u2_4; }						inline float& u2_4() { return m_u2_4; }
	inline const float& v2_4() const { return m_v2_4; }						inline float& v2_4() { return m_v2_4; }

	inline void Set0(const MatchSet2D &data, const ushort &iSrc) { data.GetMatch(iSrc, u1_0(), v1_0(), u2_0(), v2_0()); }
	inline void Set1(const MatchSet2D &data, const ushort &iSrc) { data.GetMatch(iSrc, u1_1(), v1_1(), u2_1(), v2_1()); }
	inline void Set2(const MatchSet2D &data, const ushort &iSrc) { data.GetMatch(iSrc, u1_2(), v1_2(), u2_2(), v2_2()); }
	inline void Set3(const MatchSet2D &data, const ushort &iSrc) { data.GetMatch(iSrc, u1_3(), v1_3(), u2_3(), v2_3()); }
	inline void Set4(const MatchSet2D &data, const ushort &iSrc) { data.GetMatch(iSrc, u1_4(), v1_4(), u2_4(), v2_4()); }
	inline void SaveB(const char *fileName) const
	{
		FILE *fp = fopen(fileName, "wb");
		const uint five = 5;
		...
		fwrite(&u1_0(), sizeof(float), 1, fp);		fwrite(&v1_0(), sizeof(float), 1, fp);
		fwrite(&u1_1(), sizeof(float), 1, fp);		fwrite(&v1_1(), sizeof(float), 1, fp);
		fwrite(&u1_2(), sizeof(float), 1, fp);		fwrite(&v1_2(), sizeof(float), 1, fp);
		fwrite(&u1_3(), sizeof(float), 1, fp);		fwrite(&v1_3(), sizeof(float), 1, fp);
		fwrite(&u1_4(), sizeof(float), 1, fp);		fwrite(&v1_4(), sizeof(float), 1, fp);

		fwrite(&five, sizeof(uint), 1, fp);
		fwrite(&u2_0(), sizeof(float), 1, fp);		fwrite(&v2_0(), sizeof(float), 1, fp);
		fwrite(&u2_1(), sizeof(float), 1, fp);		fwrite(&v2_1(), sizeof(float), 1, fp);
		fwrite(&u2_2(), sizeof(float), 1, fp);		fwrite(&v2_2(), sizeof(float), 1, fp);
		fwrite(&u2_3(), sizeof(float), 1, fp);		fwrite(&v2_3(), sizeof(float), 1, fp);
		fwrite(&u2_4(), sizeof(float), 1, fp);		fwrite(&v2_4(), sizeof(float), 1, fp);
...
public:
	...
	inline const float* u1s() const { return m_u1s; }			inline float* u1s() { return m_u1s; }
	inline const float* v1s() const { return m_v1s; }			inline float* v1s() { return m_v1s; }
	inline const float* u2s() const { return m_u2s; }			inline float* u2s() { return m_u2s; }
	inline const float* v2s() const { return m_v2s; }			inline float* v2s() { return m_v2s; }
	inline const  ENFT_SSE::__m128& u1_0123() const { return m_u1_0123; }	inline  ENFT_SSE::__m128& u1_0123() { return m_u1_0123; }
	inline const  ENFT_SSE::__m128& u1_4567() const { return m_u1_4567; }	inline  ENFT_SSE::__m128& u1_4567() { return m_u1_4567; }
	inline const  ENFT_SSE::__m128& v1_0123() const { return m_v1_0123; }	inline  ENFT_SSE::__m128& v1_0123() { return m_v1_0123; }
	inline const  ENFT_SSE::__m128& v1_4567() const { return m_v1_4567; }	inline  ENFT_SSE::__m128& v1_4567() { return m_v1_4567; }
	inline const  ENFT_SSE::__m128& u2_0123() const { return m_u2_0123; }	inline  ENFT_SSE::__m128& u2_0123() { return m_u2_0123; }
	inline const  ENFT_SSE::__m128& u2_4567() const { return m_u2_4567; }	inline  ENFT_SSE::__m128& u2_4567() { return m_u2_4567; }
	inline const  ENFT_SSE::__m128& v2_0123() const { return m_v2_0123; }	inline  ENFT_SSE::__m128& v2_0123() { return m_v2_0123; }
	inline const  ENFT_SSE::__m128& v2_4567() const { return m_v2_4567; }	inline  ENFT_SSE::__m128& v2_4567() { return m_v2_4567; }
	inline const  ENFT_SSE::__m128& mean_u1v1u2v2() const { return m_mean_u1v1u2v2; }
	inline const float& scale1() const { return m_scale1; }
	inline const float& scale2() const { return m_scale2; }
	...
	inline void Normalize( ENFT_SSE::__m128 *work)
	{
		...
		work[0] =  ENFT_SSE::_mm_set1_ps(m_scale1);
		m_u1_0123 =  ENFT_SSE::_mm_mul_ps(m_u1_0123, work[0]);				m_u1_4567 =  ENFT_SSE::_mm_mul_ps(m_u1_4567, work[0]);
		m_v1_0123 =  ENFT_SSE::_mm_mul_ps(m_v1_0123, work[0]);				m_v1_4567 =  ENFT_SSE::_mm_mul_ps(m_v1_4567, work[0]);
		...
		work[0] =  ENFT_SSE::_mm_set1_ps(m_scale2);
		m_u2_0123 =  ENFT_SSE::_mm_mul_ps(m_u2_0123, work[0]);				m_u2_4567 =  ENFT_SSE::_mm_mul_ps(m_u2_4567, work[0]);
		m_v2_0123 =  ENFT_SSE::_mm_mul_ps(m_v2_0123, work[0]);				m_v2_4567 =  ENFT_SSE::_mm_mul_ps(m_v2_4567, work[0]);
	}
...
public:
	...
	inline const Point3D& X(const ushort &i) const { return m_Xs[i]; }
	inline const Point2D& x(const ushort &i) const { return m_xs[i]; }
	inline const Point3D* Xs() const { return m_Xs; }
	inline const Point2D* xs() const { return m_xs; }

	inline const Point3D& cX() const { return m_cX; }	inline const  ENFT_SSE::__m128& sX() const { return m_sX; }
	inline const Point2D& cx() const { return m_cx; }	inline const  ENFT_SSE::__m128& sx() const { return m_sx; }
...
public:
	...
	inline void Resize(const ushort &N) { m_Xs.Resize(N); m_xs.Resize(N); }
	inline void Reserve(const ushort &N) { m_Xs.Reserve(N); m_xs.Reserve(N); }
	inline const ushort Size() const { return m_Xs.Size(); }
	inline const AlignedVector<Point3D>& Xs() const { return m_Xs; }
	inline		 AlignedVector<Point3D>& Xs()		{ return m_Xs; }
	inline const Point3D& X(const ushort &i) const { return m_Xs[i]; }
	inline		 Point3D& X(const ushort &i)	   { return m_Xs[i]; }
	...
	inline void Set(const ushort &i, const Point3D &X, const Point2D &x) { m_Xs[i] = X; m_xs[i] = x; }
...
public:
	...
	inline void Set(const ushort &i, const Point3D &X1, const Point3D &X2) { m_pX1s[i] = &X1; m_pX2s[i] = &X2; }
	inline const Point3D& X1(const ushort &i) const { return *m_pX1s[i]; }
	inline const Point3D& X2(const ushort &i) const { return *m_pX2s[i]; }
...
public:
	...
	inline const Point3D& X1(const uint &i) const { return m_X1s[i]; }	inline Point3D& X1(const uint &i) { return m_X1s[i]; }
	inline const Point3D& X2(const uint &i) const { return m_X2s[i]; }	inline Point3D& X2(const uint &i) { return m_X2s[i]; }
	...
	inline const uint Size() const { return m_X1s.Size(); }
	inline void Resize(const uint &N) { m_X1s.Resize(N); m_X2s.Resize(N); }
