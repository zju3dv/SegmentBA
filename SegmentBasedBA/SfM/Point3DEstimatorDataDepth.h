	...
	virtual double ComputeSSE(const Point3D &X)
	{
		...
		if(AreWeightsValid())
		{
			for(ushort ix2 = 0, ix2p1 = 1; ix2 < _N; ix2 += 2, ix2p1 += 2, ++px)
			{
				...
				e1 =  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_setr_ps(m_ws[ix2], m_ws[ix2], m_ws[ix2p1], m_ws[ix2p1]), e1);
	...
	virtual void ConstructNormalEquation(const Point3D &X, LA::AlignedMatrix3f &A, LA::AlignedVector3f &b, LA::AlignedVector3f &s)
	{
		...
		if(AreWeightsValid())
		{
			...
			for(ushort i = 0; i < N; ++i)
			{
				...
				if(m_ds[i] == 0.0f)
				{
					J1.M_00_01_02_x() =  ENFT_SSE::_mm_mul_ps(wZcI,  ENFT_SSE::_mm_sub_ps(C.r_00_01_02_x(),  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(x.X()), C.r_20_21_22_x())));
					J1.M_10_11_12_x() =  ENFT_SSE::_mm_mul_ps(wZcI,  ENFT_SSE::_mm_sub_ps(C.r_10_11_12_x(),  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(x.Y()), C.r_20_21_22_x())));
				...
				{
					J2.M_00_01_02_x() =  ENFT_SSE::_mm_mul_ps(wZcI,  ENFT_SSE::_mm_sub_ps(C.r_00_01_02_x(),  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(x.X()), C.r_20_21_22_x())));
					J2.M_10_11_12_x() =  ENFT_SSE::_mm_mul_ps(wZcI,  ENFT_SSE::_mm_sub_ps(C.r_10_11_12_x(),  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(x.Y()), C.r_20_21_22_x())));
		...
		else
		{
			...
			for(ushort i = 0; i < N; ++i)
			{
				...
				if(m_ds[i] == 0.0f)
				{
					J1.M_00_01_02_x() =  ENFT_SSE::_mm_mul_ps(ZcI,  ENFT_SSE::_mm_sub_ps(C.r_00_01_02_x(),  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(x.X()), C.r_20_21_22_x())));
					J1.M_10_11_12_x() =  ENFT_SSE::_mm_mul_ps(ZcI,  ENFT_SSE::_mm_sub_ps(C.r_10_11_12_x(),  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(x.Y()), C.r_20_21_22_x())));
					e1.x() = m_xs[i].x() - x.X();
					e1.y() = m_xs[i].y() - x.Y();
					LA::AddATAToUpper(J1, A);
					LA::AddATBTo(J1, e1, b);
				...
				{
					J2.M_00_01_02_x() =  ENFT_SSE::_mm_mul_ps(ZcI,  ENFT_SSE::_mm_sub_ps(C.r_00_01_02_x(),  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(x.X()), C.r_20_21_22_x())));
					J2.M_10_11_12_x() =  ENFT_SSE::_mm_mul_ps(ZcI,  ENFT_SSE::_mm_sub_ps(C.r_10_11_12_x(),  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(x.Y()), C.r_20_21_22_x())));
					J2.M_20_21_22_x() =  ENFT_SSE::_mm_mul_ps(w, C.r_20_21_22_x());
					e2.X() = m_xs[i].x() - x.X();
					e2.Y() = m_xs[i].y() - x.Y();
					e2.Z() = m_depthWeight * (m_ds[i] - x.Z());
