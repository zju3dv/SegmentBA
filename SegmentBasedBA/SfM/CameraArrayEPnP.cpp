...
bool CameraArrayEPnP::Run(const CameraArrayEstimatorMinimalSample &data, CameraArray &CA, AlignedVector< ENFT_SSE::__m128> &work)
{
...
bool CameraArrayEPnP::Run(const CameraArrayEstimatorData &data, CameraArray &CA, AlignedVector< ENFT_SSE::__m128> &work)
{
	if(!ComputeCws(data, work) || !ComputeAlphas(data, work))
		return false;
	ConstructLinearSystem(data, work);
...
bool CameraArrayEPnP::ComputeCws(const CameraArrayEstimatorMinimalSample &data, AlignedVector< ENFT_SSE::__m128> &work)
{
	Point3D &Cw0 = m_Cws[0];
	Cw0.XYZx() =  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_add_ps(data.X(0).XYZx(), data.X(1).XYZx()),  ENFT_SSE::_mm_add_ps(data.X(2).XYZx(), data.X(3).XYZx())), 
									    ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_add_ps(data.X(4).XYZx(), data.X(5).XYZx()),  ENFT_SSE::_mm_add_ps(data.X(6).XYZx(), data.X(7).XYZx()))),  ENFT_SSE::_mm_set1_ps(0.125f));
...
#define SET_AT_3x8(i)\
	...
	ks =  ENFT_SSE::_mm_sqrt_ps( ENFT_SSE::_mm_mul_ps(PTR_TO_SSE(m_w.Data()),  ENFT_SSE::_mm_set1_ps(0.125f)));

	k =  ENFT_SSE::_mm_set1_ps(ks.m128_f32[0]);	k.m128_f32[3] = 0;
	m_pds[0] *= k;
	m_Cws[1].XYZx() =  ENFT_SSE::_mm_add_ps(Cw0.XYZx(), m_pds[0].XYZx());

	k =  ENFT_SSE::_mm_set1_ps(ks.m128_f32[1]);	k.m128_f32[3] = 0;
	m_pds[1] *= k;
	m_Cws[2].XYZx() =  ENFT_SSE::_mm_add_ps(Cw0.XYZx(), m_pds[1].XYZx());

	k =  ENFT_SSE::_mm_set1_ps(ks.m128_f32[2]);	k.m128_f32[3] = 0;
	m_pds[2] *= k;
...
bool CameraArrayEPnP::ComputeCws(const CameraArrayEstimatorData &data, AlignedVector< ENFT_SSE::__m128> &work)
{
	work.Resize(3);
	Point3D &Cw0 = m_Cws[0];
	Cw0.SetZero();
	...
	Cw0 *=  ENFT_SSE::_mm_set1_ps(1.0f / N);
	...
	memcpy(m_pds[0], m_ATA, 48);
	 ...
	 ENFT_SSE::__m128 &ks = work[0], &k = work[1];
	ks =  ENFT_SSE::_mm_sqrt_ps( ENFT_SSE::_mm_mul_ps(PTR_TO_SSE(m_w.Data()),  ENFT_SSE::_mm_set1_ps(1.0f / N)));
...
bool CameraArrayEPnP::ComputeAlphas(const CameraArrayEstimatorMinimalSample &data, AlignedVector< ENFT_SSE::__m128> &work)
{
	...
	one_over_det =  ENFT_SSE::_mm_set1_ps(1 / det);
...
bool CameraArrayEPnP::ComputeAlphas(const CameraArrayEstimatorData &data, AlignedVector< ENFT_SSE::__m128> &work)
{
	// Inverse [ pds[1]^T, pds[2]^T, pds[3]^T ]
	work.Resize(4);
	 ENFT_SSE::__m128 &T0 = work[0], &T1 = work[1], &T2 = work[2];
...
void CameraArrayEPnP::ConstructLinearSystem(const CameraArrayEstimatorMinimalSample &data, AlignedVector< ENFT_SSE::__m128> &work)
{
	m_MT.Resize(16);
	m_b.Resize(16);
...
#define SET_MT_TOW_ROWS(i, r0, r1)\
	...
	{\
		...
		ax =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(x.x()));		SET_MT_ENTRIES(2, 5, 8, 11, r0, ax);\
													SET_MT_ENTRIES(0, 3, 6,  9, r1, zero);\
													SET_MT_ENTRIES(1, 4, 7, 10, r1, na);\
		ay =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(x.y()));		SET_MT_ENTRIES(2, 5, 8, 11, r1, ay);\
		m_b[r0] = m_b[r1] = 0;\
	}\
	else\
	{\
		...
		cx =  ENFT_SSE::_mm_sub_ps( ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(x.x()), T.r_20_21_22_x()), T.r_00_01_02_x());\
		acx =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(cx.m128_f32[0]));	SET_MT_ENTRIES(0, 3, 6,  9, r0, acx);\
		acx =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(cx.m128_f32[1]));	SET_MT_ENTRIES(1, 4, 7, 10, r0, acx);\
		acx =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(cx.m128_f32[2]));	SET_MT_ENTRIES(2, 5, 8, 11, r0, acx);\
		m_b[r0] = T.tX() - x.x() * T.tZ();\
		cy =  ENFT_SSE::_mm_sub_ps( ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(x.y()), T.r_20_21_22_x()), T.r_10_11_12_x());\
		acy =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(cy.m128_f32[0]));	SET_MT_ENTRIES(0, 3, 6,  9, r1, acy);\
		acy =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(cy.m128_f32[1]));	SET_MT_ENTRIES(1, 4, 7, 10, r1, acy);\
		acy =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(cy.m128_f32[2]));	SET_MT_ENTRIES(2, 5, 8, 11, r1, acy);\
...
void CameraArrayEPnP::ConstructLinearSystem(const CameraArrayEstimatorData &data, AlignedVector< ENFT_SSE::__m128> &work)
{
	...
	for(ushort i = 0, ix2 = 0, ix2p1 = 1; i < N; ++i, ix2 = (i << 1), ix2p1 = ix2 + 1)
	{
		...
		if((iCam = data.GetCameraIndex(i)) == 0)
		{
			na =  ENFT_SSE::_mm_sub_ps(zero, a);					SET_MT_ENTRIES(0, 3, 6,  9, ix2, na);
														SET_MT_ENTRIES(1, 4, 7, 10, ix2, zero);
			ax =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(x.x()));		SET_MT_ENTRIES(2, 5, 8, 11, ix2, ax);
														SET_MT_ENTRIES(0, 3, 6,  9, ix2p1, zero);
														SET_MT_ENTRIES(1, 4, 7, 10, ix2p1, na);
			ay =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(x.y()));		SET_MT_ENTRIES(2, 5, 8, 11, ix2p1, ay);
			m_b[ix2] = m_b[ix2p1] = 0;
		}
		else
		//iCam = data.GetCameraIndex(i);
		{
			const RigidTransformation3D &T = data.GetCalibrationParameter().GetRelativePose(iCam);
			cx =  ENFT_SSE::_mm_sub_ps( ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(x.x()), T.r_20_21_22_x()), T.r_00_01_02_x());
			acx =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(cx.m128_f32[0]));	SET_MT_ENTRIES(0, 3, 6,  9, ix2, acx);
			acx =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(cx.m128_f32[1]));	SET_MT_ENTRIES(1, 4, 7, 10, ix2, acx);
			acx =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(cx.m128_f32[2]));	SET_MT_ENTRIES(2, 5, 8, 11, ix2, acx);
			m_b[ix2] = T.tX() - x.x() * T.tZ();
			cy =  ENFT_SSE::_mm_sub_ps( ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(x.y()), T.r_20_21_22_x()), T.r_10_11_12_x());
			acy =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(cy.m128_f32[0]));	SET_MT_ENTRIES(0, 3, 6,  9, ix2p1, acy);
			acy =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(cy.m128_f32[1]));	SET_MT_ENTRIES(1, 4, 7, 10, ix2p1, acy);
			acy =  ENFT_SSE::_mm_mul_ps(a,  ENFT_SSE::_mm_set1_ps(cy.m128_f32[2]));	SET_MT_ENTRIES(2, 5, 8, 11, ix2p1, acy);
...
bool CameraArrayEPnP::ComputeXcs(const CameraArrayCalibrationParameter &calibParam, const CameraIndex *iCams)
{
	...
	for(ushort i = 0; i < N; ++i)
	{
		...
		m_Xcs[i].XYZx() =  ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps(m_Ccs[0].XYZx(),  ENFT_SSE::_mm_set1_ps(alpha.v0())),  ENFT_SSE::_mm_mul_ps(m_Ccs[1].XYZx(),  ENFT_SSE::_mm_set1_ps(alpha.v1()))), 
									  ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps(m_Ccs[2].XYZx(),  ENFT_SSE::_mm_set1_ps(alpha.v2())),  ENFT_SSE::_mm_mul_ps(m_Ccs[3].XYZx(),  ENFT_SSE::_mm_set1_ps(alpha.v3()))));
		m_Xcs[i].reserve() = 1;
		if(iCams[0] == 0)
			pos = (m_Xcs[i].Z() > 0);
		else
...
bool CameraArrayEPnP::Debug(const CameraArrayEstimatorData &data, const CameraArray &CA, AlignedVector< ENFT_SSE::__m128> &work)
{
	if(!ComputeCws(data, work) || !ComputeAlphas(data, work))
		return false;
	...
	for(ushort i = 0; i < N; ++i)
	{
		...
		Xf.XYZx() =  ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(a.v0()), Ccs[0].XYZx()),  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(a.v1()), Ccs[1].XYZx())), 
							    ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(a.v2()), Ccs[2].XYZx()),  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_set1_ps(a.v3()), Ccs[3].XYZx())));
		Xf.reserve() = 1;
		iCam = data.GetCameraIndex(i);
		if(iCam == 0)
		{
