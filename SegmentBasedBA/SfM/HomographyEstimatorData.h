...
public:
	...
	inline void FinishSettingWeights()
	{
		...
		const  ENFT_SSE::__m128 s =  ENFT_SSE::_mm_set1_ps(1 / ENFT_SSE::SSE::Sum0123(wSum));
	...
	virtual double ComputeSSE(const Homography &H)
	{
		...
		if(m_w2s.Empty())
		{
			...
			for(ushort iu1 = 0, iv1 = 1, iu2 = 2, iv2 = 3; iu1 < nPacks; iu1 += 4, iv1 += 4, iu2 += 4, iv2 += 4)
			{
				...
				ey =  ENFT_SSE::_mm_div_ps(one,  ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps(h[6], u1),  ENFT_SSE::_mm_mul_ps(h[7], v1)), h[8]));
				ex =  ENFT_SSE::_mm_sub_ps( ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps(h[0], u1),  ENFT_SSE::_mm_mul_ps(h[1], v1)), h[2]), ey), u2);
				ey =  ENFT_SSE::_mm_sub_ps( ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps(h[3], u1),  ENFT_SSE::_mm_mul_ps(h[4], v1)), h[5]), ey), v2);
				sum =  ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps(ex, ex),  ENFT_SSE::_mm_mul_ps(ey, ey)), sum);
			}
	...
	virtual void ConstructNormalEquation(const Homography &H, LA::AlignedMatrix8f &A, LA::AlignedVector8f &b, LA::AlignedVector8f &s)
	{
		...
		if(m_w2s.Empty())
		{
			...
			o =  ENFT_SSE::_mm_set1_ps(1.0f);
			const ushort nPacks = GetPacksNumber(), nRems = GetRemindersNumber();
			for(ushort iu1 = 0, iv1 = 1, iu2 = 2, iv2 = 3; iu1 < nPacks; iu1 += 4, iv1 += 4, iu2 += 4, iv2 += 4)
			{
				const  ENFT_SSE::__m128 &u = GetPack(iu1), &v = GetPack(iv1);
				x =  ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps(h[0], u),  ENFT_SSE::_mm_mul_ps(h[1], v)), h[2]);
				y =  ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps(h[3], u),  ENFT_SSE::_mm_mul_ps(h[4], v)), h[5]);
				o_z =  ENFT_SSE::_mm_div_ps(o,  ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps(h[6], u),  ENFT_SSE::_mm_mul_ps(h[7], v)), h[8]));
				o_z2 =  ENFT_SSE::_mm_mul_ps(o_z, o_z);
				x2py2_z2 =  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_add_ps( ENFT_SSE::_mm_mul_ps(x, x),  ENFT_SSE::_mm_mul_ps(y, y)), o_z2);
