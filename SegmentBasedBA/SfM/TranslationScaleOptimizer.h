...
public:
	...
	inline void Run(TranslationScaleEstimatorData &data, float &scale, const ushort &maxNumIters, AlignedVector< ENFT_SSE::__m128> &work, const bool &verbose = false)
	{
		...
		*p = 0;
		slevmar_der(func, jacf, p, (float *) data.xs().Data(), 1, Nx2, int(maxNumIters), NULL, info, workf, NULL, this);
	...
	static inline void PrintLMInfo(const float *info)
	{
		...
		printf("  %d iterations: %e --> %e, stop by %s\n", ushort(info[5]), info[0], info[1], stop.c_str());
	}
...
private:
	...
	inline void Normalize(TranslationScaleEstimatorData &data, float &scale)
	{
		...
		m_norm =  ENFT_SSE::_mm_set1_ps((1 / dMed) / 0.5f);
		m_norm.m128_f32[3] = 1;
	...
	inline void DenormalizeScale(float &scale) { scale /= m_norm.m128_f32[0]; }

	inline void Initialize(const TranslationScaleEstimatorData &data, const float &scaleInit)
	{
		m_pData = &data;
		m_pScale = &scaleInit;
	}
	inline const float& UpdateScale(const float* const &p) { m_scale = *m_pScale + p[0]; return m_scale; }
	...
	inline void Get_hx(float* const &hx) { memcpy(hx, m_xs, (m_pData->Size() << 3)); }
