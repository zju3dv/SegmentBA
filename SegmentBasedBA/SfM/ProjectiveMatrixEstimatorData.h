...
public:
	...
	virtual void NormalizeData(const float &dataNormalizeMedian, ProjectiveMatrix &P)
	{
		...
		//if(1)
		{
			m_sX = m_sx =  ENFT_SSE::_mm_set1_ps(1.0f);
			m_cX.SetZero();
			m_cx.SetZero();
		}
		else
	...
	virtual void ConstructNormalEquation(const ProjectiveMatrix &P, LA::AlignedMatrix11f &A, LA::AlignedVector11f &b, LA::AlignedVector11f &s)
	{
		...
		for(ushort i = 0; i < N; ++i)
		{
			...
			P.Project(X, ZcI, x);
			m_work[0] =  ENFT_SSE::_mm_set1_ps(ZcI);
			m_work[1] =  ENFT_SSE::_mm_set1_ps(-x.x() * ZcI);
			m_work[2] =  ENFT_SSE::_mm_set1_ps(-x.y() * ZcI);
		...
		LA::MakeReciprocal(s);
		LA::MakeSquareRoot(s);
		//s.v0123() = s.v4567() = s.v8910x() =  ENFT_SSE::_mm_set1_ps(1.0f);
...
public:
	...
	virtual void NormalizeData(const float &dataNormalizeMedian, ProjectiveMatrixMetric &P)
	{
		...
		else
		{
			...
			// Normalize focal
			m_sx =  ENFT_SSE::_mm_set1_ps(dataNormalizeMedian / P.f());
			P.f() *= m_sx.m128_f32[0];
