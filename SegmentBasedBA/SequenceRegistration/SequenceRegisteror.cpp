...
void SequenceRegisteror::Initialize(const SequenceSet &seqs, const std::string paramFileName, const bool distortionInvalid)
{
	Configurator param;
...
void SequenceRegisteror::Run(SequenceSet &seqs, const std::string outputFileName, bool viewSequences, const std::string tmpFileNameKF, const std::string tmpFileNameRelativePoses, 
							 const std::string tmpFileNameLocal, const std::string tmpFileNameGlobal, const std::string tmpFileNameBA, FILE *fpTiming)
{
	...
	std::string tmpFileName = tmpFileNameKF == "" ? "" : seqs.GetDirectory() + tmpFileNameKF;
	if(tmpFileName == "" || !LoadB(tmpFileName.c_str(), seqs, m_seqsKF, m_iFrmsListKF, m_iTrksListIdvKF, m_iTrksCmnKF))
	{
		ExtractKeyFrameSequences(seqs, m_seqsKF, m_iFrmsListKF, m_iTrksListIdvKF, m_iTrksCmnKF);
		...
		if(tmpFileName != "")
			SaveB(tmpFileName.c_str(), seqs, m_seqsKF, m_iFrmsListKF, m_iTrksListIdvKF, m_iTrksCmnKF);
	}
	//exit(0);

	Timer timer(5);

	std::vector<AlignedVector<RigidTransformation3D> > TsList(seqs.GetSequencesNumber());
	tmpFileName = tmpFileNameRelativePoses == "" ? "" : seqs.GetDirectory() + tmpFileNameRelativePoses;
	if(m_spsfmInconsistencyWeightGradient != 1.0f && (tmpFileName == "" || !LoadRelativePoses(tmpFileName.c_str(), TsList)))
	{
#if VERBOSE_SEQUENCE_REGISTRATION
		printf("****************************************************************\n");
#endif
...
//#if _DEBUG
	...
	timer.Start(4);
	RegisterOriginalSequences(m_seqsKF, m_iFrmsListKF, m_iTrksListIdvKF, m_iTrksCmnKF, seqs);
	...
	BundleAdjustmentIndividual(seqs, m_iFrmsListKF, m_baLocalMaxNumIters, m_baLocalStopMSE, m_baLocalStopRelativeReduction);
	timer.Stop(4);
	...
	if(fpTiming)
	{
		fprintf(fpTiming, "Relative pose:     ");		timer.PrintTotalTiming(0, fpTiming);
		fprintf(fpTiming, "Local:             ");		timer.PrintTotalTiming(1, fpTiming);
		fprintf(fpTiming, "Global:            ");		timer.PrintTotalTiming(2, fpTiming);
		fprintf(fpTiming, "Segment based SfM: ");		timer.PrintTotalTiming(3, fpTiming);
		fprintf(fpTiming, "Interpolation:     ");		timer.PrintTotalTiming(4, fpTiming);
	}
	else
	{
		printf("Relative pose:     ");					timer.PrintTotalTiming(0);
		printf("Local:             ");					timer.PrintTotalTiming(1);
		printf("Global:            ");					timer.PrintTotalTiming(2);
		printf("Segment based SfM: ");					timer.PrintTotalTiming(3);
		printf("Interpolation:     ");					timer.PrintTotalTiming(4);
	}
...
void SequenceRegisteror::Initialize(const SequenceSet &seqs, const Configurator &param, const bool distortionInvalid)
{
	m_kfMinNumCmnTrksTwoSeqs = TrackIndex(param.GetArgument("kf_min_common_tracks_number_two_sequences", 300));
	m_kfMinNumTrksPerKeyFrm = FeatureIndex(param.GetArgument("kf_min_tracks_number_per_key_frame", 300));
	...
	m_kfMinTrkLenKeyFrm = FrameIndex(param.GetArgument("kf_min_track_length_key_frame", 2));
	m_kfNumBinsX = ubyte(param.GetArgument("kf_bins_number_x", 8));
	m_kfNumBinsY = ubyte(param.GetArgument("kf_bins_number_y", 8));
	m_kfMinNumFtrsPerBin = FeatureIndex(param.GetArgument("kf_min_features_number_per_bin", 2));

	m_sfmSeqMaxNumTransformations = ushort(param.GetArgument("sfm_max_sequence_transformations_number", 2));
	m_sfmSeqInliersMinNum = TrackIndex(param.GetArgument("sfm_min_sequence_inliers_number", 20));
	const float sfmSeqInliersMinRatioStart = param.GetArgument("sfm_min_sequence_inliers_ratio_start", 0.9f);
	const float sfmSeqInliersMinRatioStep = param.GetArgument("sfm_min_sequence_inliers_ratio_step", -0.1f);
	const float sfmSeqInliersMinRatioEnd = param.GetArgument("sfm_min_sequence_inliers_ratio_end", 0.5f);
	m_sfmSeqInliersRatioThs.resize(0);
	for(float sfmSeqInliersMinRatio = sfmSeqInliersMinRatioStart; sfmSeqInliersMinRatio >= sfmSeqInliersMinRatioEnd; sfmSeqInliersMinRatio += sfmSeqInliersMinRatioStep)
		m_sfmSeqInliersRatioThs.push_back(sfmSeqInliersMinRatio);
	SequenceErrorLevel::SetHighestOutlierLevel(ubyte(m_sfmSeqInliersRatioThs.size() - 1));
	...
	const float errThReprojStart = param.GetArgument("error_threshold_reprojection_start", 4.0f);
	const float errThReprojStep = param.GetArgument("error_threshold_reprojection_step", 2.0f);
	const float errThReprojEnd = param.GetArgument("error_threshold_reprojection_end", 10.0f);
	m_sfmReprojErrSqThs.resize(0);
	for(float errTh = errThReprojStart; errTh <= errThReprojEnd; errTh += errThReprojStep)
		m_sfmReprojErrSqThs.push_back(errTh * errTh);
	SequenceErrorLevel::SetHighestReprojectionErrorLevel(ubyte(m_sfmReprojErrSqThs.size() - 1));

	const float errThReprojInternal = param.GetArgument("error_threshold_reprojection_internal", 1.0f);
	...
	m_sfmMinNumPtsPerPlane = param.GetArgument("sfm_min_points_number_per_plane", 100);
	...
	m_Sestor.m_ransacMinNumIters = uint(param.GetArgument("ransac_min_iterations_number_similarity", 100));
	m_Sestor.m_ransacMaxNumIters = uint(param.GetArgument("ransac_max_iterations_number_similarity", 1000));
	m_Cestor.m_ransacMinNumIters = uint(param.GetArgument("ransac_min_iterations_number_camera", 100));
	m_Cestor.m_ransacMaxNumIters = uint(param.GetArgument("ransac_max_iterations_number_camera", 300));
	//m_Pestor.m_ransacMaxNumIters = uint(param.GetArgument("ransac_max_iterations_number_plane", 3000));

	m_baaelLocalMaxNumIters = uint(param.GetArgument("baael_max_iterations_number_local", 5));
	m_baaelGlobalMaxNumIters = uint(param.GetArgument("baael_max_iterations_number_global", 3));

	m_ba3D.m_lmStopMSE = m_gto.m_lmStopMSE = 0;
	m_ba3D.m_lmMaxNumIters = m_gto.m_lmMaxNumIters = uint(param.GetArgument("ba_3d_max_iterations_number", 100));
	m_baIdvMinNumTrksPerFrm = FeatureIndex(param.GetArgument("ba_individual_min_tracks_number_per_frame", 300));
	m_baLocalMaxNumAdditionalAdjustedSeqs = SequenceIndex(param.GetArgument("ba_local_max_additional_adjusted_sequences_number", 100));
	m_baLocalMaxNumIters = uint(param.GetArgument("ba_local_max_iterations_number", 20));
	m_baLocalStopMSE = param.GetArgument("ba_local_stop_mse", 0.2f);
	m_baLocalStopRelativeReduction = param.GetArgument("ba_local_stop_relative_reduction", 0.001f);
	m_baGlobalMaxNumIters = uint(param.GetArgument("ba_global_max_iterations_number", 50));
	m_baGlobalStopMSE = param.GetArgument("ba_global_stop_mse", 0.1f);
	m_baGlobalStopRelativeReduction = param.GetArgument("ba_global_stop_relative_reduction", 0.0001f);
	...
	m_spsfmMaxNumIters = ushort(param.GetArgument("spsfm_max_iterations_number", 10));
	m_spsfmSegsNumPerSeqInit = SegmentIndex(param.GetArgument("spsfm_segments_number_per_sequence_initial", 1));
	m_spsfmSegsNumPerSeqFactor = SegmentIndex(param.GetArgument("spsfm_segments_number_per_sequence_factor", 2));
	m_spsfmSegsNumPerSeqIncr = SegmentIndex(param.GetArgument("spsfm_segments_number_per_sequence_increment", 2));
	m_spsfmInconsistencyWeightGradient = param.GetArgument("spsfm_inconsistency_weight_gradient", 0.5f);

	m_stop = param.GetArgument("stop", 0) != 0;
	m_view = param.GetArgument("view", 1) != 0;
}

void SequenceRegisteror::ExtractKeyFrameSequences(SequenceSet &seqs, SequenceSet &seqsKF, std::vector<FrameIndexList> &iFrmsListKF, 
												  std::vector<TrackIndexList> &iTrksListIdvKF, TrackIndexList &iTrksCmnKF)
{
	...
	{
		candidateCmnTrks[iTrkCmn].Set(iTrkCmn, SequenceIndex(seqs.GetCommonTrackIndividualTrackIndexList(iTrkCmn).size()), 
			seqs.CountCommonTrackInlierIndividualTracks(iTrkCmn), seqs.CountCommonTrackInlierMeasurements(iTrkCmn));
	}
	...
	for(iSeq = 0; iSeq < nSeqs; ++iSeq)
	{
		...
		for(iTrkIdv = 0; iTrkIdv < nTrksIdv; ++iTrkIdv)
		{
			...
			if((seq.GetTrackState(iTrkIdv) & FLAG_TRACK_STATE_INLIER) && ((trkLen = seq.CountTrackMarkedFrameInlierMeasurements(iTrkIdv, frmMarksKF)) >= m_kfMinTrkLenKeyFrm
			|| (iTrkCmn = seqs.GetIndividualTrackCommonTrack(iSeq, iTrkIdv)) != INVALID_TRACK_INDEX && m_cmnTrkMarks[iTrkCmn] && trkLen >= 2))
				iTrksIdvKF.push_back(iTrkIdv);
		}
		seq.DenormalizeMeasurements();
		seq.MarkKeyFrameTracks(iFrmsListKF[iSeq], frmMarksKF, iTrksIdvKF, m_kfNumBinsX, m_kfNumBinsY, m_kfMinNumFtrsPerBin, m_kfMinNumTrksPerKeyFrm, 
	...
	for(iSeq = 0; iSeq < nSeqs; ++iSeq)
	{
		...
		{
			if((seq.GetTrackState(iTrkIdv) & FLAG_TRACK_STATE_INLIER) && seq.CountTrackMarkedFrameInlierMeasurements(iTrkIdv, frmMarks) >= m_sfmMinTrkLenKeyFrm)
				iTrksIdvKF.push_back(iTrkIdv);
		}
		seq.MarkKeyFrameTracks(iFrmsListKF[iSeq], iTrksIdvKF, m_sfmMinNumTrksPerKeyFrm, m_sfmMinNumCmnTrksTwoKeyFrms, m_sfmMinNumCmnTrksThreeKeyFrms, trkMarks);
		iTrksIdvKF.resize(0);
		for(TrackIndex iTrkIdv = 0; iTrkIdv < nTrksIdv; ++iTrkIdv)
		{
			if(trkMarks[iTrkIdv])
	...
	seqs.GetSubSequences(iFrmsListKF, iTrksListIdvKF, seqsKF, iTrksCmnKF, false, true);
	for(iSeq = 0; iSeq < nSeqs; ++iSeq)
...
void SequenceRegisteror::RegisterOriginalSequences(const SequenceSet &seqsKF, const std::vector<FrameIndexList> &iFrmsListKF, 
												   const std::vector<TrackIndexList> &iTrksListIdvKF, TrackIndexList &iTrksCmnKF, SequenceSet &seqs)
{
	...
	for(iSeq = 0; iSeq < nSeqs; ++iSeq)
	{
		...
		m_Cestor.m_ransacErrorThreshold = m_Xestor.m_ransacErrorThreshold = m_sfmReprojErrSqThs.front() * seq.GetIntrinsicMatrix().one_over_fxy();
		if(seq.GetIntrinsicType() == Sequence::INTRINSIC_CONSTANT)
...
void SequenceRegisteror::BundleAdjustmentIndividual(SequenceSet &seqs, const std::vector<FrameIndexList> &iFrmsListFix, const uint &maxNumIters, const float &stopMSE, 
													const float &stopRelativeReduction)
{
	...
	for(SequenceIndex iSeq = 0; iSeq < nSeqs; ++iSeq)
	{
		...
		if(!m_iFrmsAdj.empty())
		{
			SelectAdjustedTracks(seq, m_iFrmsAdj, m_iTrksBA, m_iTrksIdv, m_baIdvMinNumTrksPerFrm);
			if(intrinsicType == Sequence::INTRINSIC_VARIABLE)
			{
				seq.GetBundleAdjustorData(m_iFrmsAdj, m_iTrksBA, m_baDataIntrinsicVariable, m_iFrmsBA);
				const FrameIndex nFrmsFixBA = FrameIndex(m_iFrmsBA.size() - m_iFrmsAdj.size());
				m_baIntrinsicVariable.Run(m_baDataIntrinsicVariable, nFrmsFixBA, false, VERBOSE_SEQUENCE_REGISTRATION ? 2 : 0);
				seq.SetBunbleAdjustmentResults(m_baDataIntrinsicVariable, nFrmsFixBA, m_iFrmsBA, m_iTrksBA);
			}
			else
			{
...
void SequenceRegisteror::SelectAdjustedTracks(const Sequence &seq, const FrameIndexList &iFrmsAdj, TrackIndexList &iTrksAdj, TrackIndexList &iTrksUnadj, 
											  const FeatureIndex minNumTrksPerFrm)
{
	...
	for(FrameIndex i = 0; i < nFrmsAdj; ++i)
	{
		...
		{
			if((iTrk = iTrks[iFtr]) == INVALID_TRACK_INDEX || m_trkMarks[iTrk] || !(seq.GetTrackState(iTrk) & FLAG_TRACK_STATE_INLIER) || (meaStates[iFtr] & FLAG_MEASUREMENT_STATE_OUTLIER))
				continue;
	...
	const float dotMin = m_candidateTrks.empty() ? 1.0f : m_candidateTrks.front().GetRayAngleDot();
	const float dotMax = m_candidateTrks.empty() ? 1.0f : i == nTrksCandidate ? std::min(m_candidateTrks.back().GetRayAngleDot(), 1.0f) : 
	...
	printf("  Ray angle: max = %f, min = %f\n", acos(dotMin) * FACTOR_RAD_TO_DEG, acos(dotMax) * FACTOR_RAD_TO_DEG);
#endif
}

void SequenceRegisteror::AdjustPoints(const TrackIndexList &iTrks, Sequence &seq)
{
	Point3D X;
