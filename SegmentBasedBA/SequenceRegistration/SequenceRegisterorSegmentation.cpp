...
void SequenceRegisteror::SegmentSequence(const AlignedVector<RigidTransformation3D> &Ts, const SequenceIndex &iSeq, SequenceSet &seqs, const SegmentIndex &nSegs)
{
...
static inline bool ComputeGradient(const SequenceSet &seqs, const SequenceIndex &iSeq, FrameIndex &iFrm, LA::AlignedVector7f &g)
{
	...
	{
		if((iTrkIdv = iTrksIdv[iFtr]) == INVALID_TRACK_INDEX || !(seq.GetTrackState(iTrkIdv) & FLAG_TRACK_STATE_INLIER) || (meaStates[iFtr] & FLAG_MEASUREMENT_STATE_OUTLIER))
			continue;
		...
		J_x_Xseg =  ENFT_SSE::_mm_sub_ps( ENFT_SSE::_mm_mul_ps(invZc, C.r_00_01_02_x()),  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_mul_ps(invZc,  ENFT_SSE::_mm_set1_ps(e.x())), C.r_20_21_22_x()));
		J_y_Xseg =  ENFT_SSE::_mm_sub_ps( ENFT_SSE::_mm_mul_ps(invZc, C.r_10_11_12_x()),  ENFT_SSE::_mm_mul_ps( ENFT_SSE::_mm_mul_ps(invZc,  ENFT_SSE::_mm_set1_ps(e.y())), C.r_20_21_22_x()));
...
void SequenceRegisteror::EstimateConsecutiveFrameInconsistencies_Gradient(const SequenceSet &seqs, const SequenceIndex &iSeq, std::vector<float> &inconsistencies)
{
	FrameIndex iFrm1, iFrm2;
	bool b1, b2;
